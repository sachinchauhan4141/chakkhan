...
const App = () => {
  ...
  // Clean exit back to main menu — no reload, won't trigger beforeunload
  const exitToMenu = () => {
  ...
  const {
    ...
    gameMode, isOnline, roomCode, connectedPlayers, localPlayerRole, rollResult,
    isPaused, pauseCount
  ...
  // Prevent accidental page reload/tab close during an active game
  React.useEffect(() => {
    if (gameMode === 'MENU' || gameState === 'GAME_OVER') return;
    const handler = (e) => {
      e.preventDefault();
      e.returnValue = ''; // Required for Chrome — triggers the browser's native confirm
    };
  ...
  return (
    <div className="min-h-screen w-full flex flex-col bg-slate-900 text-slate-100 overflow-hidden">
      ...
      <header className="w-full flex items-center justify-between px-4 py-3 border-b border-slate-700/60 bg-slate-800/60 backdrop-blur-md shrink-0 relative z-40">
        ...
        {/* Available moves chips (left side) */}
        <div className="flex items-center gap-1 min-w-[60px]">
          {availableMoves.filter(m => m !== 'CAPTURE_BONUS').map((m, i) => (
            <span key={i}
              className="w-6 h-6 rounded-full bg-slate-700 border border-slate-500 text-white text-[10px] font-bold flex items-center justify-center">
              {m}
            </span>
          ))}
        </div>

        {/* Current turn + last roll (center) */}
          ...
          {menuOpen && (
            <>
              ...
              <div className="absolute right-0 top-full mt-2 w-44 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl overflow-hidden">
                <button
                  onClick={() => { dispatch(togglePause()); setMenuOpen(false); }}
      ...
      {/* Main area — blocked while paused */}
      <main className={`flex-1 flex flex-col items-center justify-center px-2 py-3 gap-2 sm:gap-3 overflow-auto transition-opacity duration-300 ${isPaused ? 'opacity-30 pointer-events-none select-none' : 'opacity-100'}`}>
        ...
        <div className="w-full max-w-[520px] flex justify-between px-1">
          <PlayerYard player="p3" />
          <PlayerYard player="p4" />
        </div>
        ...
        {/* Bottom yards */}
        <div className="w-full max-w-[520px] flex justify-between px-1">
          <PlayerYard player="p2" />
          <PlayerYard player="p1" />
        </div>

      </main>

      {/* Sticks animation overlay */}
      <ObjectSticks />
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 text-center max-w-xs w-full mx-4 shadow-2xl">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 rounded-xl bg-slate-700 hover:bg-slate-600 text-slate-200 font-semibold text-sm transition-all">
                Stay
              </button>
              <button
                onClick={exitToMenu}
...
if (typeof document !== 'undefined') {
  ...
  s.innerHTML = `
    ...
    @keyframes stickTumble {
      0%   { opacity:0; transform: translateY(-60px) rotate(30deg) scale(0.6); }
      60%  { opacity:1; transform: translateY(8px) rotate(-10deg) scale(1.05); }
